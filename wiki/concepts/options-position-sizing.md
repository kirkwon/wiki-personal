
--
# Options Position Sizing

Position sizing in options trading must account for theta exposure and risk tolerance. Proper position sizing is non-negotiable for success in options trading.^[raw/articles/Options Trading Guide.md]

## Theta-Aware Position Sizing

**Long Options**:
- Capital allocation: 1–5% of portfolio per option position
- Small positions due to theta risk
- Allow 3–6 months for thesis to play out
- Diversify across multiple long option positions

**Short Options**:
- Capital allocation: 5–15% of portfolio per short position
- Larger positions possible due to theta benefit
- Use defined risk strategies only
- Maintain sufficient margin buffer

**Cash Reserves**: 65% of portfolio in cash for flexibility and risk management.

## Example Portfolio Allocation

- **LEAPS Positions**: 10% of portfolio (long-term growth)
- **Straddle Positions**: 5% of portfolio (volatility plays)
- **Covered Calls**: 15% of portfolio (income generation)
- **Iron Condors**: 5% of portfolio (range-bound markets)
- **Cash Reserves**: 65% (flexibility and risk management)

## Risk Management Limits

- **Single Position Limit**: 5% of total portfolio
- **Strategy Limit**: 15–20% of portfolio per strategy type
- **Sector Diversification**: No more than 25% in single sector
- **Time Diversification**: Mix of short-term and long-term positions

## Capital Allocation by Risk Profile

- Conservative portfolio: 10–20% in options
- Moderate portfolio: 20–40% in options
- Aggressive portfolio: 40–60% in options
- Speculative portfolio: 5–10% in high-risk options

Position sizing connects to [[theta-time-decay]] and all major strategy pages.

See also: [[options-fundamentals]]

See also: [[risk-assessment-framework]]

See also: [[LEAPS-options]]

See also: [[covered-calls]]

See also: [[straddle-strategies]]

See also: [[ITM-vs-OTM-options]]
