#!/usr/bin/env python3
"""
Automated tests for the Skills Dashboard.
Validates data integrity, HTML functionality, and visualization.
"""
...
def test_data_integrity():
    ...
    if not overlap_skills.issubset(all_skills_set):
        issues.append(f"❌ Overlaps has {overlap_skills - all_skills_set} not in all_skills")
    else:
    ...
    if not multi_skills.issubset(all_skills_set):
        issues.append(f"❌ Multi-tag has {multi_skills - all_skills_set} not in all_skills")
    else:
    ...
    if not desc_skills.issubset(all_skills_set):
        issues.append(f"❌ Descriptions has {desc_skills - all_skills_set} not in all_skills")
    else:
...
def test_no_duplicate_tags():
    ...
    # Check each skill has unique tags
    duplicates = []
    for skill, tags in skill_tags.items():
        if len(tags) != len(list(tags)):  # Shouldn't happen with set, but checking
            duplicates.append(skill)

    if duplicates:
        print(f"❌ Found duplicate tags in: {duplicates[:5]}")
        return False

    print(f"✅ No duplicate tags found across {len(skill_tags)} skills")
...
def test_network_graph_data():
    ...
    if tag_count < 3:
        issues.append(f"❌ Only {tag_count} tags - need at least 3 for network")
        return False
    else:
    ...
    if total_links < 5:
        issues.append("⚠️  Few links - network may be sparse")

    if issues:
        for issue in issues:
            print(issue)
        # Don't fail on warnings
        return issues[0].startswith('❌')
...
def test_skill_quality():
    ...
    data = load_data()
    generic_terms = {'mindset', 'attitude', 'belief', 'success', 'failure',
                    'time', 'money', 'energy', 'focus', 'power', 'value'}
    ...
    for skill in data['all_skills']:
        ...
        if skill_lower in generic_terms:
            found_generic.append(skill)

    if found_generic:
        print(f"⚠️  Found {len(found_generic)} generic terms:")
        for skill in found_generic[:5]:
            print(f"   • {skill}")
        print("   (These may be valid if they're part of a longer phrase)")
    else:
        print(f"✅ No standalone generic terms found")

    # Check skills are actionable (contain verbs or are techniques)
    action_words = {'making', 'building', 'breaking', 'applying', 'using',
                   'decision', 'techniques', 'thinking', 'analysis', 'training',
                   'revision', 'assessment', 'adaptive', 'analogical'}
...
def test_html_structure():
    """Test 6: HTML Structure and Tables"""
    ...
    if len(stat_cards) >= 4:
        ...
        issues.append(f"❌ Only found {len(stat_cards)} stat cards (need 4+)")
    ...
    if len(tabs) >= 6:
        ...
        issues.append(f"❌ Only found {len(tabs)} tabs (need 6+)")
